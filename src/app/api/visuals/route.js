  ...
  try {
    ...
    const payload = await idempotent(requestKey, fingerprint, async () => {
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        while (attempt < 2) {
          try {
            const prompt = buildPrompt(product, variationIndex, 5);
            // In the official @google/genai SDK, image generation uses generateImages.
            // Note: Imagen 3 doesn't typically accept reference images for generation,
            // so we rely entirely on the highly descriptive prompt generated by buildPrompt.
            const response = await ai.models.generateImages({
              ...
              config: {
                ...
                aspectRatio: '3:4', // 4:5 is not natively supported by all models, 3:4 is standard
              },
            });
            const imageBytes = response.generatedImages?.[0]?.image?.imageBytes;
            if (!imageBytes) throw new Error('No image was returned.');
            visuals.push(await optimizeGeneratedImage(imageBytes));
            break;
          } catch (error) {
